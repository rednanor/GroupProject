...
public class EventCauseConfig extends SuperConfig {
	...
	public EventCauseConfig(String fname) {
		super(fname);
		initialise();
	}
	...
	private void initialise(){
			...
			Sheet currentSheet = workbook.getSheet(ColumnIndexes.EVENTCAUSE__SHEETNO);
			...
			for (int i = 1; i < currentSheet.getRows(); i++) {
				...
				if (row.length > 0) {
					...
					createEventCause(Integer.parseInt(row[ColumnIndexes.EVENTCAUSE_CAUSECODE_COLNO].getContents()), 
							Integer.parseInt(row[ColumnIndexes.EVENTCAUSE_EVENTID_COLNO].getContents()), 
							row[ColumnIndexes.EVENTCAUSE_DESCRIPTION_COLNO].getContents());
