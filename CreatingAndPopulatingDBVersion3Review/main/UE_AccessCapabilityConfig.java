...
public class UE_AccessCapabilityConfig extends SuperConfig{
	...
	public UE_AccessCapabilityConfig(String fname) {
		super(fname);
		initialise();
	}
			...
			for (int i = 1; i < currentSheet.getRows(); i++) {
				...
				if (row.length > 0) {
					...
					for (int j = 0; j < indivCapabilities.length; j++) {
						int accessID=PersistenceUtil.findAccessCapability(indivCapabilities[j]).getAccessID();
						UE_AccessCapability ue_acc=new UE_AccessCapability(row[ColumnIndexes.UE_TAC_COLNO].getContents(),accessID);
