...
public class UserEquipmentConfig extends SuperConfig {
	...
	public UserEquipmentConfig(String fname) {
		super(fname);
		initialise();
	}
	...
	private void initialise(){
		...
		for (int i = 1; i < currentSheet.getRows(); i++) {
			...
			if (row.length > 0) {
				...
				String manufacturer=row[ColumnIndexes.UE_MANUFACTURER_COLNO].getContents();
				int manufacturerId=PersistenceUtil.findManufacturerByName(manufacturer).getManufacturerID();
				int model=PersistenceUtil.findUEModelByName(row[ColumnIndexes.UE_MODEL_COLNO].getContents()).getModelID();

				createUserEquipment((row[ColumnIndexes.UE_TAC_COLNO].getContents()),manufacturerId,
						model,row[ColumnIndexes.UE_UETYPE_COLNO].getContents(),row[ColumnIndexes.UE_OS_COLNO].getContents(),
